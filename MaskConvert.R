...
#bop <- raster("InputData/Rasters/BoP_Aligned/BType1.tif")
# load NCC mask
#ncc <- readOGR(dsn = "Boundary", layer = "NCC_Area_20mbuffer")
# Mask - assigns NA values outside of region polygon
#out <- "AlignedData/Rasters/BType1.tif"
...
maskconv <- function(rasterfile, datatype){
  ...
  if(file.exists(inf)){
    ...
    bopmask <- raster("AlignedData/Rasters/BType1.tif")
...
rastersnum <- c("Chla_mean","distLand","FBPI","fetchMax","fetchMean","fetchNW","fetchSE","MBPI",
                ...
                "spr_MnSp","spr_Sal","spr_Stress","spr_Temp","ArcRug","bathy","BBPI")

## List of INT1U integer rasters
rasters1int <- c("Bloom_Freq","BType2","DepthCode")
